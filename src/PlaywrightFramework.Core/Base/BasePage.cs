...
public abstract class BasePage
{
    ...
    /// <summary>
    /// Double clicks an element by ILocator
    /// </summary>
    /// <param name="locator">Element locator</param>
    /// <param name="options">Double click options</param>
    public async Task DoubleClickAsync(ILocator locator, LocatorDblClickOptions? options = null)
    {
    ...
    /// <summary>
    /// Right clicks an element by ILocator
    /// </summary>
    /// <param name="locator">Element locator</param>
    ...
    public async Task<IReadOnlyList<string>> SelectOptionAsync(string locator, string[] values, PageSelectOptionOptions? options = null)
    {
        ...
        Logger.LogDebug("Selected options in element: {Locator} - {SelectedValues}", locator, string.Join(", ", selectedValues));
    ...
    /// <param name="options">Select option options</param>
    /// <returns>Array of selected option values</returns>
    public async Task<IReadOnlyList<string>> SelectOptionAsync(string locator, string value, PageSelectOptionOptions? options = null)
    {
    ...
    public async Task WaitForElementAsync(string locator, int? timeout = null)
    {
        ...
        await Page.WaitForSelectorAsync(locator, new PageWaitForSelectorOptions { Timeout = timeoutMs });
    ...
    public async Task WaitForElementToBeHiddenAsync(string locator, int? timeout = null)
    {
        ...
        await Page.WaitForSelectorAsync(locator, new PageWaitForSelectorOptions { State = WaitForSelectorState.Hidden, Timeout = timeoutMs });
    ...
    private async Task HighlightElementAsync(string selector, int? durationMs = null)
    {
        if (!Config.Execution.HighlightElements)
            return;
        try
        {
            var duration = durationMs ?? Config.Execution.HighlightDurationMs;
            var borderWidth = Config.Execution.HighlightBorderWidth;
            var highlightScript = $@"
                ...
                if (element) {{
                    ...
                    element.style.border = '{borderWidth}px solid red';
            ...
            ";
            await Page.EvaluateAsync(highlightScript);
            Logger.LogDebug("Highlighted element with red border: {Selector} for {Duration}ms", selector, duration);
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Failed to highlight element: {Selector}", selector);
        }
    }

    private async Task HighlightLocatorAsync(ILocator locator, int? durationMs = null)
    {
        if (!Config.Execution.HighlightElements)
            return;
        try
        {
            ...
            Logger.LogDebug("Highlighted locator for {Duration}ms", duration);
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Failed to highlight locator");
        }
    }
}